...
class Bitmap
  ...
  def initialize(width, height=nil)
    @entity = if width.is_a? String
      ...
      filepath = RGSS.get_file(filename)
			puts "Loading bitmap: #{filepath}"
			ChunkyPNG::Image.from_file(filepath)
    else
  ...
  def blt(x, y, src_bitmap, src_rect, opacity=255)
		src_rect.height.times do |h|
			src_rect.width.times do |w|
				@entity[x+w, y+h] = src_bitmap.entity[src_rect.x+w, src_rect.y+h]
			end
		end
  ...
  def fill_rect(x, y, width=nil, height=nil, color=nil)
    if x.is_a? Rect
      ...
      height = rect.height
		...
		color = ChunkyPNG::Color.rgba(color.red, color.green, color.blue, color.alpha)
    @entity.rect(x, y, x + width, y + height, ChunkyPNG::Color::TRANSPARENT, color)
  end
