...
class Tilemap
  ...
  attr_reader :map_events       # The data of the map
  ...
  def map_data=(value)
	...
	def map_events=(value)
		return if @map_events == value
		@map_events = value
	end
  ...
  def refresh
    ...
    for i in 0...@map_data.xsize
      for j in 0...@map_data.ysize
		...
		# Event Graphics
    bitmap = Bitmap.new(@map_data.xsize * 32, @map_data.ysize * 32)
		@map_events.each do |i,e|
				# puts "Processing Event: #{e.name}"
				painted = false
				e.pages.each do |p|
					begin
						next if p.graphic.tile_id == 0 && p.graphic.character_name.empty?
						next if painted
						...
						unless p.graphic.character_name.empty?
							cName = p.graphic.character_name
							cBitmap = Cache.character(cName)
							sign = cName[/^[\!\$]./]
							...
							index = p.graphic.character_index
							pattern = p.graphic.pattern < 3 ? p.graphic.pattern : 1
							sx = (index % 4 * 3 + pattern) * cw
							sy = (index / 4 * 4 + (p.graphic.direction - 2) / 2) * ch
							src_rect = Rect.new(sx, sy, cw, ch)

							# n = p.graphic.character_index
							# src_rect = Rect.new((n%4*3+1)*cw, (n/4*4)*ch, cw, ch)
							character = Bitmap.new(cw, ch)
							character.blt(0, 0, cBitmap, src_rect)
							bitmap.blt(x, y, character, Rect.new(0, 0, cw, ch))
						else
							b = get_bitmap(p.graphic.tile_id)
							bitmap.blt(x, y, b, rect)
						end
					rescue => e
						puts e.inspect
					end
    ...
    # create_shadow_bitmap
		...
		out = @layers[0].bitmap.entity.clone
		# out.compose!(@layers[5].bitmap.entity)
		out.compose!(bitmap.entity)
		out.compose!(@layers[1].bitmap.entity)
		out.compose!(@layers[2].bitmap.entity)

		# puts "Saving..."
		out.save("#{mapName}.png", :interlace => true)
  end
  #--------------------------------------------------------------------------
      ...
      for j in 0...@map_data.ysize
        if !projects_shadow?(@map_data[i,j,0])
          if projects_shadow?(@map_data[i-1,j,0]) && (j == 0 || projects_shadow?(@map_data[i-1,j-1,0]))
            bmp.fill_rect(i * 32, j * 32, 16, 31, Color.new(0,0,0,128))
          end
          # if i + 1 < @map_data.xsize && projects_shadow?(@map_data[i+1,j,0]) && (j == 0 || projects_shadow?(@map_data[i+1,j-1,0]))
            # bmp.fill_rect(i * 48, j * 32, 16, 31, Color.new(0,0,0,128))
          # end
        end
      end
    end
